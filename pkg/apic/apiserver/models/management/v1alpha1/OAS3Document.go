/*
 * This file is automatically generated
 */

package v1alpha1

import (
	"encoding/json"

	apiv1 "github.com/Axway/agent-sdk/pkg/apic/apiserver/models/api/v1"
)

var (
	_OAS3DocumentGVK = apiv1.GroupVersionKind{
		GroupKind: apiv1.GroupKind{
			Group: "management",
			Kind:  "OAS3Document",
		},
		APIVersion: "v1alpha1",
	}
)

const (
	OAS3DocumentScope = "VirtualAPI"

	OAS3DocumentResource = "oas3documents"
)

func OAS3DocumentGVK() apiv1.GroupVersionKind {
	return _OAS3DocumentGVK
}

func init() {
	apiv1.RegisterGVK(_OAS3DocumentGVK, OAS3DocumentScope, OAS3DocumentResource)
}

// OAS3Document Resource
type OAS3Document struct {
	apiv1.ResourceMeta

	Owner struct{} `json:"owner"`

	Spec struct{} `json:"spec"`
}

// FromInstance converts a ResourceInstance to a OAS3Document
func (res *OAS3Document) FromInstance(ri *apiv1.ResourceInstance) error {
	if ri == nil {
		res = nil
		return nil
	}

	m, err := json.Marshal(ri.Spec)
	if err != nil {
		return err
	}

	spec := &struct{}{}
	err = json.Unmarshal(m, spec)
	if err != nil {
		return err
	}

	*res = OAS3Document{ResourceMeta: ri.ResourceMeta, Spec: *spec}

	return err
}

// AsInstance converts a OAS3Document to a ResourceInstance
func (res *OAS3Document) AsInstance() (*apiv1.ResourceInstance, error) {
	m, err := json.Marshal(res.Spec)
	if err != nil {
		return nil, err
	}

	spec := map[string]interface{}{}
	err = json.Unmarshal(m, &spec)
	if err != nil {
		return nil, err
	}

	meta := res.ResourceMeta
	meta.GroupVersionKind = OAS3DocumentGVK()

	return &apiv1.ResourceInstance{ResourceMeta: meta, Spec: spec}, nil
}
